...
static double g_keep_alive = 90.0;
...
static void *readwrite_routine(void *arg)
{
	...
	for (;;)
	{
		...
		for (;;)
		{
			double keep_alive = difftime(time(NULL), co->keep_alive);
			if (keep_alive <= g_keep_alive)
			{
				if (g_mqttcontroller.protocol_handler(co, buf, sizeof(buf)))
				{
					struct pollfd pf = {0};
					pf.fd = fd;
			...
			{
				printf("fd: %d, timediff: %lf\n", fd, keep_alive);
			}

			printf("readwrite_routine: close(fd:%d), g_readwrite size=%d\n", fd, g_readwrite.size());
			co->fd = -1;
			close(fd);
			break;
		}
